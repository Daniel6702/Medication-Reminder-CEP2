...
class Cep2Model:
    ...
    def actuators_list(self) -> List[Cep2ZigbeeDevice]:
        return list(filter(lambda s: s.type_ in {"led", "power plug", "switch", "light"},
    ...
    def sensors_list(self) -> List[Cep2ZigbeeDevice]:
        return list(filter(lambda s: s.type_ in {"pir", "numeric"},
                           self.__devices.values()))
    ...
    def remove(self, device: Union[Cep2ZigbeeDevice, List[Cep2ZigbeeDevice]]) -> None:
        """ Remove devices from the database.

        Args:
            device (Union[Cep2ZigbeeDevice, List[Cep2ZigbeeDevice]]): a device object, or a list of
            device objects to remove.
        """
        # If the value given as an argument is a Cep2ZigbeeDevice, then create a list with it.
        list_devices = [device] if isinstance(device, Cep2ZigbeeDevice) else device

        # Remove the devices with the given IDs.
        for s in list_devices:
            if s.id_ in self.__devices:
                del self.__devices[s.id_]
